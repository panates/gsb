...
function mergeCalls(fns) {
  ...
  if (typeof fns === 'function')
    return fns;
  const stack = Array.isArray(fns) ? fns.slice() : [fns];
  if (!stack.length)
    return;

  const wrappedCall = (cb, ...args) => {
    ...
    const handleResult = (err, v) => {
      ...
      if (v)
        return cb(null, v);
      process.nextTick(() => wrappedCall(cb, ...args));
    };
  ...
  return (...args) => {
    return promisify.fromCallback(cb => wrappedCall(cb, ...args));
  };
}
