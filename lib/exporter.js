...
function exportSchema(schema, options) {
  ...
  const exportEnumType = (name, typ) => {
    ...
    const copyFrom = (src) => {
      ...
      if (src.values) {
        ...
        Object.keys(o.values).forEach(f => {
          o.values[f].value = o.values[f].value || f;
        });
      }
    };
  ...
  const exportObjectFields = (srcFields, dstFields, sch, kind, typName) => {
    Object.keys(srcFields).forEach(n => {
      ...
      if (kind && kind === 'object') {
        ...
        if (src.filter) {
          ...
          Object.keys(src.filter).forEach(n => {
        ...
        if (src.sort) {
          ...
          src.sort.forEach(n => {
            ...
            if (!m)
              throw new SchemaError('"%s" does not match required format for "sort" property', n);
        ...
        if (src.limit != null) {
          delete dst.limit;
          dst.args = dst.args || {};
          dst.args.limit = {type: 'Int', defaultValue: src.limit};
        }
  ...
  if (gqlFormat && out.typeDefs) {
    if (options.format === EXPORT_GQL_PURE)
      out.resolvers = {};
    Object.keys(out.typeDefs).forEach(n => {
      ...
      switch (t.kind) {
        ...
        case 'object': {
          ...
          /* istanbul ignore else */
          if (t.fields) {
            Object.keys(t.fields).forEach(nn => {
              const f = t.fields[nn];
