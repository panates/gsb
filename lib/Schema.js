...
class Schema {
  ...
  load(def, options, loadFn) {
    ...
    this.namespace = (typeof def === 'string') ? def : path.dirname(caller());
  ...
  _beforeLoad(def, schema, options) {
    if (def.link) {
      ...
      const rootDir = path.resolve(options.rootPath || './', './' + nsPath);
      ...
      for (const lnk of a) {
        ...
        if (path.basename(lnk) === '*') {
          ...
          for (let i = 0; i < files.length; i++) {
            const f = path.join(nsPath, lnkDir, path.basename(files[i], path.extname(files[i])));
            if (b.indexOf(f) < 0)
              b.push(f);
          }
        } else {
          b.push(lnk.match(/^[\\/].+$/) ? lnk : path.join(nsPath, lnk));
        }
      }
      def.link = b;
    }
  ...
  _mapFile(namespace, schema, options) {
    return path.resolve(options.rootPath, path.normalize('./' + namespace));
  }
