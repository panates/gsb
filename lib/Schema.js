...
class Schema {
          ...
          return Promise.resolve(
              ...
              .then(lnk => {
                ...
                if (typeof lnk === 'string')
                  return Schema._fromFile(this, lnk, options, context)
                      .then(schemas => this.link(...schemas));
  ...
  /**
   *
   * @param {Schema} [currSchema]
   * @param {string} pattern
   * @param {Object} options
   * @param {*} [context]
   * @return {Promise<Array<Schema>>}
   * @private
   * @static
   */
  static _fromFile(currSchema, pattern, options, context) {
    const result = [];
    return Promise.resolve().then(() => {
      if (typeof pattern !== 'string')
        throw new Error('You must provide file path');

      const rootSchema = currSchema && (currSchema._root || currSchema);
      const currPath = (currSchema && currSchema._location &&
          path.dirname(currSchema._location)) || '';
      const rootPath = (options && options.rootPath) ||
          (rootSchema && rootSchema._location &&
              path.dirname(rootSchema._location)) || '';

      if (pattern.startsWith('#'))
        pattern = path.resolve(
            path.join(rootPath || currPath, pattern.substring(1)));
      else if (!path.isAbsolute(pattern))
        pattern = path.resolve(path.join(currPath || rootPath, pattern));
      ...
      return waterfall.every(files,
          (next, f) => {
            ...
            if (typeof obj !== 'object')
              throw new ErrorEx('Can\'t load schema file "%s". This file does not export an object instance', f);
            obj.location = f;
            return this._fromObject(rootSchema, obj, options, context)
                .then(sch => result.push(sch));
          }
      );
